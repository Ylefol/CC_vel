...
conditions = (adata.obs['phase'] == 'G1') & (adata.obs['angles'] > s_angle) & (adata.obs['angles'] < g2m_angle)
adata.obs['phase']=adata.obs['phase'].cat.add_categories(['G1_in_S'])
adata.obs.loc[conditions, 'phase'] = 'G1_in_S'

conditions = (adata.obs['phase']=='G1') & (adata.obs['angles'] >g2m_angle)
adata.obs['phase']=adata.obs['phase'].cat.add_categories(['G1_in_G2M'])
adata.obs.loc[conditions, 'phase'] = 'G1_in_G2M'




 
S_adata=adata[adata.obs['phase'].isin(['G1','G1_in_S'])]
sc.tl.rank_genes_groups(S_adata, 'phase', method='t-test', key_added = "t-test")
df1 = sc.get.rank_genes_groups_df(S_adata, group='G1_in_S', key='t-test', pval_cutoff=0.01,log2fc_min=1)
df2 = sc.get.rank_genes_groups_df(S_adata, group='G1_in_S', key='t-test', pval_cutoff=0.01,log2fc_max=-1)
G1_in_S_genes=pd.concat([df1, df2], ignore_index = True)
...
G2M_adata=adata[adata.obs['phase'].isin(['G1','G1_in_G2M'])]
sc.tl.rank_genes_groups(G2M_adata, 'phase', method='t-test', key_added = "t-test")

df1 = sc.get.rank_genes_groups_df(G2M_adata, group='G1_in_G2M', key='t-test', pval_cutoff=0.01,log2fc_min=1)
df2 = sc.get.rank_genes_groups_df(G2M_adata, group='G1_in_G2M', key='t-test', pval_cutoff=0.01,log2fc_max=-1)
G1_in_G2M_genes=pd.concat([df1, df2], ignore_index = True)
...
with pd.ExcelWriter("G1_infiltration.xlsx") as writer:
    G1_in_S_genes.to_excel(writer, sheet_name="S_cluster_S_genes", index=False)
    G1_in_G2M_genes.to_excel(writer, sheet_name="G2M_cluster_G2M_genes", index=False)
...
cell_line='HaCat'
folder='A_B'
my_delay_df=pd.read_csv('data_files/data_results/delay_genes/'+cell_line+'/'+folder+'_delay_genes.csv')
retrieved_genes=my_func.get_sig_genes(cell_line,folder,t_test_based=True,delay_type=None,variability_based=False)
test=my_delay_df[my_delay_df['gene_name'].isin(retrieved_genes)]
test=test[test['dec_to_0']<-283.33]

test.to_csv('HaCat_neg_delay.csv')
...
retrieved_genes=my_func.get_sig_genes(cell_line,folder,t_test_based=True,delay_type=None,variability_based=False)
test=my_delay_df[my_delay_df['gene_name'].isin(retrieved_genes)]
test=test[test['dec_to_0']<-288.66]

test.to_csv('jurkat_neg_delay.csv')
...
retrieved_genes=my_func.get_sig_genes(cell_line,folder,t_test_based=True,delay_type=None,variability_based=False)
test=my_delay_df[my_delay_df['gene_name'].isin(retrieved_genes)]
test=test[test['dec_to_0']<-130.46]

test.to_csv('293t_neg_delay.csv')